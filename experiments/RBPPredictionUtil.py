...
class RBPPredictionUtil(object):
    ...
    def predict_rbps(
        ...
        for file in os.listdir(hypothetical_genbank_dir):
            ...
            if 1 in preds_xgb:
                filename = (
                    file[: -len("-hypothetical-embeddings.csv")] + "-rbp-embeddings.csv"
                )
    ...
    def check_differences(self, prottransbert_complete_dir, plm_complete_dir):
            ...
            try:
                ...
                if rbp_prottransbert - rbp_plm or rbp_plm - rbp_prottransbert:
                    print(
                        phage,
                        "|",
                        rbp_prottransbert - rbp_plm,
                        "|",
                        rbp_plm - rbp_prottransbert,
                        "|",
                    )
                    erroneous.add(phage)
        ...
        extra_files = set(os.listdir(plm_complete_dir)) - set(
            os.listdir(prottransbert_complete_dir)
        )
    ...
    def generate_fasta_addl_phages(
        self, phage, prottransbert_complete_dir, dest_dir, *fasta_folders
    ):
        """
        ...
        with open(
            f"{prottransbert_complete_dir}/{phage}-rbp-embeddings.csv", "r"
        ) as embeddings:
            rbp_prottransbert, _ = util.get_proteins_embeddings(embeddings)
