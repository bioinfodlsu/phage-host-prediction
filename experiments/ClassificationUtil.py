...
class ClassificationUtil(object):
    ...
    def classify(
        ...
        # Load data
        rbp_embeddings = pd.read_csv(
            f"{constants.INPHARED}/{constants.DATA}/{constants.PLM_EMBEDDINGS_CSV[plm]}",
            low_memory=False,
        )
        rbp_embeddings["Modification Date"] = pd.to_datetime(
            rbp_embeddings["Modification Date"]
        )
        ...
        if feature_columns is None:
            unknown_hosts_X, unknown_hosts_y = self.get_unknown_hosts(
        ...
        if save_feature_importance:
            ...
            indices = [
                x + 1
                for _, x in sorted(
                    zip(feature_importances, range(len(feature_importances))),
                    reverse=True,
                )
            ]
    ...
    def classify_handpicked_embeddings(self, feature_columns, filename):
        ...
        # Load data
        boeckaerts = pd.read_csv(
            f'{constants.INPHARED}/{constants.DATA}/{constants.PLM_EMBEDDINGS_CSV["BOECKAERTS"]}',
            low_memory=False,
        )
        prott5 = pd.read_csv(
            f'{constants.INPHARED}/{constants.DATA}/{constants.PLM_EMBEDDINGS_CSV["PROTT5"]}',
            low_memory=False,
        )
        rbp_embeddings = pd.merge(
            boeckaerts, prott5, how="inner", validate="one_to_one"
        )
        ...
        counts, X_train, X_test, y_train, y_test = self.random_train_test_split(
            ...
            + [
                str(i)
                for i in range(1, prott5.shape[1] - constants.INPHARED_EXTRA_COLS + 1)
            ],
        )
