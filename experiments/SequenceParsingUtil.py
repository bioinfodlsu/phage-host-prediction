...
class SequenceParsingUtil(object):
    ...
    def generate_rbp_len_distribution_prokka(
        ...
        for phage in self.no_cds_annot:
            with open(
                ...
                + f"/{complete_genome_dir}/{phage[1]}/{phage[1]}.faa"
            ) as handle:
    ...
    def get_annot_products_prokka(self, complete_genome_dir):
        ...
        for folder in os.listdir(
            os.path.dirname(os.getcwd()) + f"/{complete_genome_dir}"
        ):
            if folder[-1] != "a":
                phages.add(folder)
        ...
        for phage in phages:
            try:
                with open(
                    os.path.dirname(os.getcwd())
                    + f"/{complete_genome_dir}/{phage}/{phage}.faa"
                ) as handle:
                    for record in SeqIO.parse(handle, "fasta"):
    ...
    def generate_rbp_hypothetical_fasta_prokka(
        self,
        complete_genome_dir,
        rbp_prokka_dir,
            ...
            with open(
                ...
                for record in SeqIO.parse(handle, "fasta"):
                    ...
                    if len(rbp_fasta_str) > 0:
                        ...
                        with open(
                            os.path.join(rbp_prokka_dir, file_name), "w"
                        ) as rbp_fasta_file:
                            rbp_fasta_file.write(rbp_fasta_str)
                    ...
                    if len(hypothetical_fasta_str) > 0:
                        ...
                        with open(
                            os.path.join(hypothetical_prokka_dir, file_name), "w"
                        ) as hypothetical_fasta_file:
                            hypothetical_fasta_file.write(hypothetical_fasta_str)
    ...
    def generate_rbp_hypothetical_nucleotide(
        self, rbp_genbank_dir, hypothetical_genbank_dir, lower_bound, upper_bound
    ):
        """
    ...
    def generate_rbp_hypothetical_nucleotide_prokka(
        self,
        complete_genome_dir,
        rbp_prokka_dir,
        hypothetical_prokka_dir,
            ...
            with open(
                ...
                for record in SeqIO.parse(handle, "fasta"):
                    ...
                    if self.__is_rbp(product, translation, lower_bound, upper_bound):
                        ...
                        fasta = (
                            os.path.dirname(os.getcwd())
                            + f"{complete_genome_dir}/{phage[1]}/{phage[1]}.ffn"
                        )
