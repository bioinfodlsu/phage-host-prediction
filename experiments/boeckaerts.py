...
def protein_features(protein_sequences):
    ...
    for item in protein_sequences:
        ...
        frac_unch_polar.append(
            (item.count("S") + item.count("T") + item.count("N") + item.count("Q"))
            / len(item)
        )
        frac_polar.append(
            (
...
def CTDD(sequence):
    ...
    for p in property:
        encoding = (
            Count2(group1[p], sequence)
            + Count2(group2[p], sequence)
            + Count2(group3[p], sequence)
        )
    return encoding
    ...
    for i in range(matrix.shape[0]):
        for j in range(i + 1, matrix.shape[1]):
            if (
                ...
                & (i not in groups_list)
                & (j not in groups_list)
                & (i not in groups_dict.keys())
            ):
                groups_dict[i] = [j]
...
def NestedGroupKFold(
    ...
    for train_outer, test_outer in outer_cv.split(X, y, groups):
        ...
        tuned_model = GridSearchCV(
            model, cv=inner_cv, param_grid=parameter_grid, scoring=scorer
        )
        ...
        recall = sum([a * b for a, b in zip(recall_list, class_weights)]) / sum(
            class_weights
        )
        precision = sum([a * b for a, b in zip(precision_list, class_weights)]) / sum(
            class_weights
        )
        ...
        if (e < treshold) & (iden_list[j] < 100):
            ...
            try:
                handle = Entrez.efetch(
                    db="protein", id=gi, rettype="gb", retmode="text"
                )
